...
async function bulk(q, table, rows, cols, conflict) {
  if (!rows.length) return;                              // tom liste gir ugyldig SQL i postgres.js
...
export async function syncCharacter(q, notify) {
  ...
  const out = { character: row.character_name, alerts: [], errors: {} };
  const now = new Date();
  // Hver del kjøres for seg: feiler én, logges den under sitt navn og de andre lagres likevel.
  const part = async (name, fn) => { try { await fn(); } catch (e) { out.errors[name] = String(e.message || e).slice(0, 200); console.error("sync", name, e); } };

  let token;
  try { token = await accessToken(q, row); }
  catch (e) { await q`update jita.sso_tokens set last_error = ${"token: " + String(e.message || e)} where character_id = ${cid}`; throw e; }
  ...
  await part("ordrer", async () => {
    ...
    for (const o of orders) {
      await q`insert into jita.my_orders (order_id, type_id, is_buy, price, volume_remain, volume_total, issued, duration, state,
              ...
              values (${o.order_id}, ${o.type_id}, ${!!o.is_buy_order}, ${o.price}, ${o.volume_remain}, ${o.volume_total}, ${o.issued}::timestamptz,
                ${o.duration}, 'open', ${now}::timestamptz, ${now}::timestamptz, ${o.escrow ?? null}::numeric, ${o.location_id}, ${String(o.range)}::text, ${o.region_id}, ${cid})
    ...
    closed = await q`update jita.my_orders set state = 'closed', last_seen = ${now}::timestamptz
                     where state = 'open' and character_id = ${cid} and not (order_id = any(${orders.map((o) => o.order_id)}::bigint[])) returning order_id, type_id, is_buy, volume_remain, volume_total`;
    out.orders = orders.length; out.closed = closed.length;
  });

  await part("transaksjoner", async () => {
    if (!txs) return;
  ...
  await part("hangar", async () => {
    if (!assets) return;                                   // hentingen feilet → behold forrige hangar
    const hangar = assets.filter((a) => a.location_flag === "Hangar" && a.location_type === "station");
    await q.begin(async (tx) => {                          // slett + skriv i én transaksjon: aldri tom hangar ved feil
      await tx`delete from jita.my_assets`;
      await bulk(tx, "jita.my_assets", hangar.map((a) => ({ item_id: a.item_id, type_id: a.type_id, quantity: a.quantity, location_id: a.location_id, location_flag: a.location_flag, seen_at: now })),
  ...
  await part("broker-sats", async () => {
    ...
    if (meas[0]?.n >= 2 && meas[0].rate > 0.005 && meas[0].rate < 0.035)
      await q`update jita.profile set broker_fee_measured = ${Number(meas[0].rate)}::numeric, broker_measured_at = now() where id = 1`;
    out.broker_measured = meas[0]?.rate ? Number(meas[0].rate) : null;
  });

  const errText = Object.entries(out.errors).map(([k, v]) => `${k}: ${v}`).join(" | ") || null;
  await q`update jita.sso_tokens set last_sync = now(), last_error = ${errText}::text where character_id = ${cid}`;
  out.ok = true;
  out.partial = Object.keys(out.errors).length > 0;
  return out;
}
...
async function syncDecisions(q, orders, closed, txs, now) {
  for (const o of orders.filter((o) => o.is_buy_order)) {
    ...
    if (d) {
      await q`update jita.decisions set price = ${o.price}, qty = ${o.volume_total}, filled_qty = ${filled || null}::int where id = ${d.id}`;
    } else {
  ...
  for (const c of closed.filter((c) => c.is_buy)) {
    ...
    if (filled > 0) await q`update jita.decisions set filled_at = coalesce(filled_at, ${now}::timestamptz), filled_qty = ${filled}::int,
                            note = concat_ws(' · ', note, ${c.volume_remain > 0 ? `${c.volume_remain} ikke fylt (utløpt/kansellert)` : "fylt"}::text) where order_id = ${c.order_id}`;
    else await q`update jita.decisions set closed_at = ${now}::timestamptz, note = concat_ws(' · ', note, 'kansellert/utløpt uten fylling') where order_id = ${c.order_id} and closed_at is null`;
  }
  ...
  for (const d of open) {
    ...
    if (s.sold >= d.n) await q`update jita.decisions set closed_at = ${now}::timestamptz, sell_price = ${s.avg}::numeric where id = ${d.id}`;
  }
}
